...
## Loading Data
```{r, prompt=FALSE}
data_raw <- read.csv("../data_sets/Material 02 - 4 - R - Biomassa - Dados.csv")
data_raw_new_cases <- read.csv("../data_sets/Material 02 - 4 - R - Biomassa - Dados - Novos Casos.csv")
```
...
set.seed(1988)
knn <- train(biomassa ~ ., data = training_data, method = "knn", tuneGrid=tuneGrid)
print(knn)
```
...
r2(prediction.knn,test_data$biomassa)
```
...
data_new_cases$biomassa <- NULL
result <- cbind(data_new_cases, biomassa=prediction.knn_new_data)
print(result)
```
